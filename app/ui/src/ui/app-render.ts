...
import { renderCarePilot } from "./views/carepilot.ts";
      ...
      <main class="content ${isChat ? "content--chat" : ""}">
        ...
        ${
          state.tab === "carepilot"
            ? renderCarePilot({
                connected: state.connected,
                loading: state.carepilotLoading,
                error: state.carepilotError,
                state: state.carepilotState,
                activeSection: state.carepilotSection,
                onboardingStep: state.carepilotOnboardingStep,
                onboardingDraft: state.carepilotOnboardingDraft,
                savingProfile: state.carepilotSavingProfile,
                privacyBusy: state.carepilotPrivacyBusy,
                lastReceipt: state.carepilotLastReceipt,
                recording: state.carepilotRecording,
                audioPreviewUrl: state.carepilotAudioPreviewUrl,
                transcriptDraft: state.carepilotTranscriptDraft,
                transcriptConfidence: state.carepilotTranscriptConfidence,
                transcriptError: state.carepilotTranscriptError,
                chatDraft: state.chatMessage,
                chatSending: state.chatSending,
                chatMessages: state.chatMessages,
                onSectionChange: (section) => {
                  state.carepilotSection = section;
                },
                onOnboardingStepChange: (step) =>
                  (state.carepilotOnboardingStep = Math.max(0, Math.min(2, step))),
                onDraftChange: (patch) => {
                  state.carepilotOnboardingDraft = {
                    ...state.carepilotOnboardingDraft,
                    ...patch,
                  };
                },
                onSaveProfile: () =>
                  (state as unknown as OpenClawApp).saveCarePilotProfile(state.carepilotOnboardingDraft),
                onStartRecording: () =>
                  void (state as unknown as OpenClawApp).startCarePilotRecording(),
                onStopRecording: () => (state as unknown as OpenClawApp).stopCarePilotRecording(),
                onTranscribe: () =>
                  void (state as unknown as OpenClawApp).transcribeCarePilotRecording(),
                onTranscriptDraftChange: (text) => {
                  state.carepilotTranscriptDraft = text;
                },
                onApplyTranscriptToChat: () => {
                  state.chatMessage = state.carepilotTranscriptDraft;
                },
                onChatDraftChange: (text) => {
                  state.chatMessage = text;
                },
                onSendChat: () =>
                  void (state as unknown as OpenClawApp).handleSendChat(state.chatMessage),
                onRefresh: () => (state as unknown as OpenClawApp).loadCarePilot(),
                onRunPrivacy: (action) =>
                  void (state as unknown as OpenClawApp).runCarePilotPrivacyAction(action),
              })
            : nothing
        }
