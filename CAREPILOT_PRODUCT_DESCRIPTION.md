...
To finalize product behavior and UI copy, these decisions are still needed:
1. First live action mode: real booking, call-to-book, or simulated booking with real search?
2. Triage sensitivity target: conservative vs balanced?
3. Primary demo persona: chronic-condition user, caregiver, or general wellness user?
4. Should in-network filtering be in MVP or postponed?
5. Should confidence scores be shown explicitly to users or translated into plain uncertainty language only?
6. Default proactive cadence: max 1/day or every 48 hours?
7. MVP profile model: single profile only or family profiles?
8. Apple Health in hackathon: real HealthKit integration on iOS demo device, or simulated synced feed with production-ready API contract?
9. Which Apple Health metrics are required for MVP v1: menstrual cycle, medication tracking, workouts only, or include sleep and heart rate too?
10. Voice mode in MVP: input-only (STT) or full voice conversation (STT + TTS)?
11. Document analysis MVP: lab reports only, or lab + imaging reports, or direct image interpretation too?
12. File retention policy: auto-delete after X days, or user-managed deletion only?
